...
#[no_mangle]
pub extern "C" fn PR_GetEnv(var: *const ffi::c_char) -> *mut ffi::c_char {
    debug_assert!(!var.is_null());
    let var = unsafe { CStr::from_ptr(var) };
    let Ok(key) = var.to_str() else {
        return ptr::null_mut();
    };
    eprintln!("PR_GetEnv: {key:?}");
    let guard = ENV_MUTEX.lock();
    let Ok(value) = env::var(key) else {
        return ptr::null_mut();
    };
    eprintln!("PR_GetEnv: {key} = {value}");
...
#[no_mangle]
pub extern "C" fn PR_GetEnvSecure(var: *const ffi::c_char) -> *mut ffi::c_char {
    unimplemented!()
}
...
#[no_mangle]
pub extern "C" fn PR_SetEnv(string: *const ffi::c_char) -> PRStatus {
    debug_assert!(!string.is_null());
    let string = unsafe { CStr::from_ptr(string) };
    eprintln!("PR_SetEnv: {string:?}");
    let Ok(key_value) = string.to_str() else {
...
#[no_mangle]
pub extern "C" fn PR_DuplicateEnvironment() -> *mut *mut ffi::c_char {
    unimplemented!()
}
