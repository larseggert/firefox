...
use std::{ffi, os::fd::AsRawFd as _};

use crate::{
    prfile::PR_FILEMETHODS,
    prinrval::PRIntervalTime,
    prio::{
        PRDescType, PRFileDesc, PRFilePrivate, PRIOMethods, PRNetAddr, PRSendFileData,
        PRTransmitFileFlags, PRWritevFN,
    },
    prtypes::{PRInt32, PRIntn},
};
...
#[no_mangle]
pub extern "C" fn PR_FileDesc2NativeHandle(fd: *mut PRFileDesc) -> PROsfd {
    debug_assert!(!fd.is_null());
    eprintln!("PR_FileDesc2NativeHandle");

    let fd = unsafe { Box::from_raw(fd) };
    let private = unsafe { Box::from_raw(fd.secret()) };
    let res = match private.as_ref() {
        PRFilePrivate::File(file) => file.as_raw_fd(),
        PRFilePrivate::PipeReader(pipe) => pipe.as_raw_fd(),
        PRFilePrivate::PipeWriter(pipe) => pipe.as_raw_fd(),
    };
...
#[no_mangle]
pub extern "C" fn PR_ChangeFileDescNativeHandle(fd: *mut PRFileDesc, osfd: PROsfd) {
    unimplemented!()
}
...
#[no_mangle]
pub extern "C" fn PR_ImportFile(osfd: PROsfd) -> *mut PRFileDesc {
    unimplemented!()
}
...
#[no_mangle]
pub extern "C" fn PR_EmulateAcceptRead(
    sd: *mut PRFileDesc,
...
#[no_mangle]
pub extern "C" fn PR_EmulateSendFile(
