...
use std::{
    ffi,
    io::{Error, ErrorKind, Seek as _, SeekFrom},
    mem, ptr,
};

use crate::{
    prerr::PR_INVALID_METHOD_ERROR,
    prerror::PR_SetError,
    prio::{
        PRAcceptFN, PRAcceptreadFN, PRBindFN, PRConnectFN, PRConnectcontinueFN, PRDescType,
        PRFileDesc, PRFileInfo, PRFileInfo64, PRFilePrivate, PRGetpeernameFN, PRGetsocketoptionFN,
        PRGetsocknameFN, PRIOMethods, PRListenFN, PRRecvFN, PRRecvfromFN, PRReservedFN,
        PRSeekWhence, PRSendFN, PRSendfileFN, PRSendtoFN, PRSetsocketoptionFN, PRShutdownFN,
        PRTransmitfileFN, PRWritevFN,
    },
    prtypes::{PRInt16, PRInt32, PRInt64, PRIntn, PROffset32, PROffset64, PRStatus},
};
...
extern "C" fn FileRead(fd: *mut PRFileDesc, buf: *mut ffi::c_void, amount: PRInt32) -> PRInt32 {
    unimplemented!()
}
...
extern "C" fn FileWrite(fd: *mut PRFileDesc, buf: *const ffi::c_void, amount: PRInt32) -> PRInt32 {
    unimplemented!()
}
...
extern "C" fn FileSeek(fd: *mut PRFileDesc, offset: PROffset32, how: PRSeekWhence) -> PROffset32 {
    unimplemented!()
}
...
extern "C" fn FileSeek64(fd: *mut PRFileDesc, offset: PROffset64, how: PRSeekWhence) -> PROffset64 {
    unimplemented!()
}
...
#[no_mangle]
extern "C" fn FileAvailable(fd: *mut PRFileDesc) -> PRInt32 {
    eprintln!("FileAvailable");
    debug_assert!(!fd.is_null());
    let fd = unsafe { Box::from_raw(fd) };
    let mut private = unsafe { Box::from_raw(fd.secret()) };
    let file = match *private {
        PRFilePrivate::File(ref mut file) => Ok(file),
    ...
    if let Ok(file) = file {
        if let Ok(cur) = file.stream_position() {
            if let Ok(end) = file.seek(SeekFrom::End(0)) {
                if let Ok(res) = file.seek(SeekFrom::Start(cur)) {
                    Box::into_raw(private);
                    Box::into_raw(fd);
                    return PRInt32::try_from(end - cur).unwrap_or(-1);
                }
            }
        }
    }
...
extern "C" fn FileAvailable64(fd: *mut PRFileDesc) -> PRInt64 {
    unimplemented!()
}
...
extern "C" fn FileGetInfo(fd: *mut PRFileDesc, info: *mut PRFileInfo) -> PRStatus {
    unimplemented!()
}
...
extern "C" fn FileGetInfo64(fd: *mut PRFileDesc, info: *mut PRFileInfo64) -> PRStatus {
    unimplemented!()
}
...
extern "C" fn FileSync(fd: *mut PRFileDesc) -> PRStatus {
    unimplemented!()
}
...
extern "C" fn FileClose(fd: *mut PRFileDesc) -> PRStatus {
    unimplemented!()
}
...
//   return in_flags;
// } /* FilePoll */
extern "C" fn FilePoll(fd: *mut PRFileDesc, in_flags: PRInt16, out_flags: *mut PRInt16) -> PRInt16 {
    unimplemented!()
}
...
#[no_mangle]
pub extern "C" fn PR_InvalidInt() -> PRIntn {
    debug_assert!(false, "I/O method is invalid");
    PR_SetError(PR_INVALID_METHOD_ERROR, 0);
    -1
}

#[no_mangle]
pub extern "C" fn PR_InvalidInt16() -> PRInt16 {
    debug_assert!(false, "I/O method is invalid");
    PR_SetError(PR_INVALID_METHOD_ERROR, 0);
    -1
}
...
#[no_mangle]
pub extern "C" fn PR_InvalidInt64() -> PRInt64 {
    debug_assert!(false, "I/O method is invalid");
    PR_SetError(PR_INVALID_METHOD_ERROR, 0);
    -1
}
...
#[no_mangle]
pub extern "C" fn PR_InvalidStatus() -> PRStatus {
    debug_assert!(false, "I/O method is invalid");
    PR_SetError(PR_INVALID_METHOD_ERROR, 0);
    PRStatus::PR_FAILURE
}
...
#[no_mangle]
pub extern "C" fn PR_InvalidDesc() -> *mut PRFileDesc {
    debug_assert!(false, "I/O method is invalid");
    PR_SetError(PR_INVALID_METHOD_ERROR, 0);
    ptr::null_mut()
}
...
#[expect(
    ...
    reason = "These are function pointers."
)]
pub static mut PR_FILEMETHODS: PRIOMethods = PRIOMethods {
    file_type: PRDescType::PR_DESC_FILE,
    close: FileClose,
    read: FileRead,
    write: FileWrite,
    available: FileAvailable,
    available64: FileAvailable64,
    fsync: FileSync,
    seek: FileSeek,
    seek64: FileSeek64,
    fileInfo: FileGetInfo,
    fileInfo64: FileGetInfo64,
    writev: unsafe { mem::transmute::<*const (), PRWritevFN>(PR_InvalidInt as *const ()) },
    connect: unsafe { mem::transmute::<*const (), PRConnectFN>(PR_InvalidStatus as *const ()) },
    accept: unsafe { mem::transmute::<*const (), PRAcceptFN>(PR_InvalidDesc as *const ()) },
    bind: unsafe { mem::transmute::<*const (), PRBindFN>(PR_InvalidStatus as *const ()) },
    listen: unsafe { mem::transmute::<*const (), PRListenFN>(PR_InvalidStatus as *const ()) },
    shutdown: unsafe { mem::transmute::<*const (), PRShutdownFN>(PR_InvalidStatus as *const ()) },
    recv: unsafe { mem::transmute::<*const (), PRRecvFN>(PR_InvalidInt as *const ()) },
    send: unsafe { mem::transmute::<*const (), PRSendFN>(PR_InvalidInt as *const ()) },
    recvfrom: unsafe { mem::transmute::<*const (), PRRecvfromFN>(PR_InvalidInt as *const ()) },
    sendto: unsafe { mem::transmute::<*const (), PRSendtoFN>(PR_InvalidInt as *const ()) },
    poll: FilePoll,
    acceptread: unsafe { mem::transmute::<*const (), PRAcceptreadFN>(PR_InvalidInt as *const ()) },
    transmitfile: unsafe {
        mem::transmute::<*const (), PRTransmitfileFN>(PR_InvalidInt as *const ())
    },
    getsockname: unsafe {
        mem::transmute::<*const (), PRGetsocknameFN>(PR_InvalidStatus as *const ())
    },
    getpeername: unsafe {
        mem::transmute::<*const (), PRGetpeernameFN>(PR_InvalidStatus as *const ())
    },
    reserved_fn_6: unsafe { mem::transmute::<*const (), PRReservedFN>(PR_InvalidInt as *const ()) },
    reserved_fn_5: unsafe { mem::transmute::<*const (), PRReservedFN>(PR_InvalidInt as *const ()) },
    getsocketoption: unsafe {
        mem::transmute::<*const (), PRGetsocketoptionFN>(PR_InvalidStatus as *const ())
    },
    setsocketoption: unsafe {
        mem::transmute::<*const (), PRSetsocketoptionFN>(PR_InvalidStatus as *const ())
    },
    sendfile: unsafe { mem::transmute::<*const (), PRSendfileFN>(PR_InvalidInt as *const ()) },
    connectcontinue: unsafe {
        mem::transmute::<*const (), PRConnectcontinueFN>(PR_InvalidStatus as *const ())
    },
    reserved_fn_3: unsafe { mem::transmute::<*const (), PRReservedFN>(PR_InvalidInt as *const ()) },
    reserved_fn_2: unsafe { mem::transmute::<*const (), PRReservedFN>(PR_InvalidInt as *const ()) },
    reserved_fn_1: unsafe { mem::transmute::<*const (), PRReservedFN>(PR_InvalidInt as *const ()) },
    reserved_fn_0: unsafe { mem::transmute::<*const (), PRReservedFN>(PR_InvalidInt as *const ()) },
};
...
/// Return the method tables for files.
#[no_mangle]
pub extern "C" fn PR_GetFileMethods() -> *mut PRIOMethods {
    eprintln!("PR_GetFileMethods");
    unsafe { &raw mut PR_FILEMETHODS }
}
