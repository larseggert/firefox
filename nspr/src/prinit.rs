...
use crate::{
    prsystem::PR_GetPageSize,
    prthread::{PRThreadPriority, PRThreadType},
    prtypes::{PRInt32, PRIntn, PRStatus, PRUintn},
};
...
#[no_mangle]
pub const extern "C" fn PR_GetVersion() -> *const ffi::c_char {
    env!("CARGO_PKG_VERSION").as_ptr().cast::<ffi::c_char>()
}
...
#[no_mangle]
pub extern "C" fn PR_Init(_type: PRThreadType, _priority: PRThreadPriority, _maxPTDs: PRUintn) {
    unimplemented!()
}
...
#[no_mangle]
pub extern "C" fn PR_Cleanup() -> PRStatus {
    unimplemented!()
}
...
#[no_mangle]
pub type PRCallOnceFN = extern "C" fn() -> PRStatus;

// typedef PRStatus (PR_CALLBACK *PRCallOnceWithArgFN)(void *arg);

#[no_mangle]
pub type PRCallOnceWithArgFN = extern "C" fn(arg: *mut ffi::c_void) -> PRStatus;

static ONCE: Mutex<()> = Mutex::new(());

#[no_mangle]
pub extern "C" fn PR_CallOnce(once: *mut PRCallOnceType, func: PRCallOnceFN) -> PRStatus {
    let guard = ONCE.lock();
    let once = unsafe { &mut *once };
    // eprintln!("PR_CallOnce {once:?}");
...
#[no_mangle]
pub extern "C" fn PR_CallOnceWithArg(
    once: *mut PRCallOnceType,
