...
impl SFVList {
    ...
    fn parse_more(&self, header: &nsACString) -> Result<(), nsresult> {
        ...
        let _: List = Parser::new(&header).parse().map_err(|_| NS_ERROR_FAILURE)?;
...
impl SFVDictionary {
    ...
    fn parse_more(&self, header: &nsACString) -> Result<(), nsresult> {
        let _: Dictionary = Parser::new(&header).parse().map_err(|_| NS_ERROR_FAILURE)?;
        Ok(())
    }
...
fn bare_item_from_interface(obj: &nsISFVBareItem) -> Result<BareItem, nsresult> {
    ...
    unsafe {
        obj.deref().GetType(&raw mut obj_type);
    }
