 ...
 *
 * Dirtying a pref schedules that asynchronous prefs.js write, but nothing
 * observable says when it has been dispatched and not yet run, which is the
 * window this test needs. So rather than timing that window, the loop below
 * asserts the invariant on every main-thread turn until the write lands. Its
 * body is deliberately free of any await other than the single yield: a
 * scheduled write can only be dispatched while the main thread is in the event
 * loop, so that yield is the only point at which the slot can be filled, and
 * the savePrefFile() immediately after it is the one that has to cope.
 */

const kPref = "test.savePrefFile.explicit.target";
const kIterations = 2;
// Enough concurrent backups to keep the pref-write task queue busy for
// milliseconds, so that a write dispatched during the yield below is still
// waiting its turn when savePrefFile() inspects the shared slot.
const kFillerCount = 4;
// Generous bound: the write is scheduled 500ms out and a turn costs a few ms.
const kMaxTurns = 400;
...
add_task(async function test_savePrefFile_explicit_target_not_coalesced() {
  let prefsJsPath = Services.dirsvc.get("PrefF", Ci.nsIFile).path;
  ...
  for (let iteration = 0; iteration < kIterations; iteration++) {
    ...
    let prefsJsWrittenAt = diskFile(prefsJsPath).lastModifiedTime;

    let turns = 0;
    let missed = false;
    let sawAsyncWrite = false;
    while (!sawAsyncWrite && !missed && turns < kMaxTurns) {
      turns++;
      ...
      sawAsyncWrite =
        diskFile(prefsJsPath).lastModifiedTime != prefsJsWrittenAt;
    }
    ...
    // Either the write landed, or it was still pending when the check above
    // caught the bug. Both mean the loop covered the window; neither happening
    // would mean it ran out of turns and proved nothing.
    Assert.ok(
      sawAsyncWrite || missed,
