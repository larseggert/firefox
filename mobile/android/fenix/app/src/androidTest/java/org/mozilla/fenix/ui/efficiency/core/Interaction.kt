...
/**
 * The one shape every element verb has.
 *
 * Forty public verbs in BasePage were the same twelve lines with three lines of difference: announce the command,
 * announce the lookup, resolve, retry once if a known overlay was covering the target, report the miss, dump the
 * screen, throw with the selector in the message - and somewhere in the middle, do the thing. That skeleton appeared 40
 * times, which is why patience drifted between verbs (mozVerify polled, mozClick did not) and why the failure text
 * differed depending on which one you called.
 *
 * Here it exists once. A verb is now a name, a policy, a predicate and an action.
 */

/** How long to keep trying before calling it absent. */
sealed class WaitPolicy {
    ...
    /** Look until the deadline. What mozVerify did. */
    data class Poll(val timeout: Long = 5_000, val interval: Long = 500) : WaitPolicy()
}

/**
 * What the executor needs from its host. BasePage supplies these; the executor knows nothing about pages, navigation or
 * Compose rules.
 */
interface VerbHost {
 ...
 *
 * Six verbs - selected, not selected, enabled, not enabled, checked, not checked - were 40 lines each and identical
 * apart from which assertion they called and whether the sense was inverted.
 */
object ElementState {

    fun isEnabled(element: Any): Boolean = probe(element, Trait.ENABLED)

    fun isSelected(element: Any): Boolean = probe(element, Trait.SELECTED)

    fun isChecked(element: Any): Boolean = probe(element, Trait.CHECKED)
     ...
     * semantics at all", so `!isEnabled` is not the same claim as `isNotEnabled`
     * - which is exactly the trap mozClickWhenEnabled fell into.
     */
...
fun VerbHost.require(
    ...
    if (element == null) {
        ...
        // Optional UI is a legitimate absence, not a failure: report it as skipped and move on.
        if (optional) {
...
private fun VerbHost.seek(
    ...
    fun once(): Any? {
        ...
        val found = via?.invoke(selector, applyPreconditions) ?: locate(selector, applyPreconditions)
    ...
    once()?.let {
        return Probe(seen, it)
    }
    if (policy is WaitPolicy.Poll) {
        val deadline = SystemClock.uptimeMillis() + policy.timeout
        while (SystemClock.uptimeMillis() < deadline) {
            SystemClock.sleep(policy.interval)
            once()?.let {
                return Probe(seen, it)
            }
        }
    }
    return Probe(seen, null)
}
...
fun VerbHost.requireAbsent(
    ...
    fun present(): Boolean =
        locate(selector, false)?.let { ElementState.probe(it, ElementState.Trait.DISPLAYED) } == true

    val timeout = (policy as? WaitPolicy.Poll)?.timeout ?: 0
    val interval = (policy as? WaitPolicy.Poll)?.interval ?: 0
    val deadline = SystemClock.uptimeMillis() + timeout
    var appeared = present()
    // sustain: keep looking for it and fail on sight. Otherwise: keep looking until it is gone.
    while (SystemClock.uptimeMillis() < deadline && appeared != sustain) {
    ...
    if (appeared) {
        val message =
            if (sustain) {
                ...
                "'${selector.description}' was expected to be absent but is visible"
            }
        rep?.endCmd(success = false, message = message)
        if (dumpOnFailure) dumpFailure("$verb failed: ${selector.description}")
