...
struct RefCountedBase {
  ...
  virtual void
  method_test3() { // expected-note {{caller function declared here}}
    test(); // expected-error {{functions marked as MOZ_CAN_RUN_SCRIPT can only
            // be called from functions also marked as MOZ_CAN_RUN_SCRIPT}}
  }
...
void test2_parent() { // expected-note {{caller function declared here}}
  test2(
      new RefCountedBase); // expected-error {{arguments must all be strong refs
                           // or caller's parameters when calling a function
                           // marked as MOZ_CAN_RUN_SCRIPT (including the
                           // implicit object argument).  'new RefCountedBase'
                           // is neither.}} \ expected-error {{functions marked
                           // as MOZ_CAN_RUN_SCRIPT can only be called from
...
MOZ_CAN_RUN_SCRIPT void test2_parent2() {
  test2(new RefCountedBase); // expected-error {{arguments must all be strong
                             // refs or caller's parameters when calling a
                             // function marked as MOZ_CAN_RUN_SCRIPT (including
                             // the implicit object argument).  'new
                             // RefCountedBase' is neither.}}
}
...
MOZ_CAN_RUN_SCRIPT void test2_parent4() {
  ...
  test2(
      refptrRefUnsafe1); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'refptrRefUnsafe1' is neither.}}

  RefPtr<RefCountedBase> &refptrRefUnsafe2 =
      refPtrInHeap ? *refPtrInHeap : refptr;
  test2(
      refptrRefUnsafe2); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'refptrRefUnsafe2' is neither.}}

  RefPtr<RefCountedBase> &refptrRefUnsafe3 =
      refPtrInHeap ? refptr : *refPtrInHeap;
  test2(
      refptrRefUnsafe3); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'refptrRefUnsafe3' is neither.}}
}
...
MOZ_CAN_RUN_SCRIPT void test2_parent6() {
  ...
  RefPtr<RefCountedBase> &refptrRefUnsafe1 = *refPtrInHeap;
  refptrRefUnsafe1
      ->method_test(); // expected-error {{arguments must all be strong refs or
                       // caller's parameters when calling a function marked as
                       // MOZ_CAN_RUN_SCRIPT (including the implicit object
                       // argument).  'refptrRefUnsafe1->' is neither.}}
  refptrRefUnsafe1
      ->method_test2(); // expected-error {{arguments must all be strong refs or
                        // caller's parameters when calling a function marked as
                        // MOZ_CAN_RUN_SCRIPT (including the implicit object
                        // argument).  'refptrRefUnsafe1->' is neither.}}
  ...
  refptrRefUnsafe2
      ->method_test(); // expected-error {{arguments must all be strong refs or
                       // caller's parameters when calling a function marked as
                       // MOZ_CAN_RUN_SCRIPT (including the implicit object
                       // argument).  'refptrRefUnsafe2->' is neither.}}
  refptrRefUnsafe2
      ->method_test2(); // expected-error {{arguments must all be strong refs or
                        // caller's parameters when calling a function marked as
                        // MOZ_CAN_RUN_SCRIPT (including the implicit object
  ...
  refptrRefUnsafe3
      ->method_test(); // expected-error {{arguments must all be strong refs or
                       // caller's parameters when calling a function marked as
                       // MOZ_CAN_RUN_SCRIPT (including the implicit object
                       // argument).  'refptrRefUnsafe3->' is neither.}}
  refptrRefUnsafe3
      ->method_test2(); // expected-error {{arguments must all be strong refs or
                        // caller's parameters when calling a function marked as
                        // MOZ_CAN_RUN_SCRIPT (including the implicit object
...
MOZ_CAN_RUN_SCRIPT void test2_parent7() {
  ...
  tRef->method_test();  // expected-error {{arguments must all be strong refs or
                        // caller's parameters when calling a function marked as
                        // MOZ_CAN_RUN_SCRIPT (including the implicit object
                        // argument).  'tRef' is neither.}}
  tRef->method_test2(); // expected-error {{arguments must all be strong refs or
                        // caller's parameters when calling a function marked as
                        // MOZ_CAN_RUN_SCRIPT (including the implicit object
                        // argument).  'tRef' is neither.}}
}
...
struct RefCountedChild : public RefCountedBase {
  virtual void method_test3()
      override; // expected-note {{overridden function declared here}}
                // expected-note {{overridden function declared here}}
                // expected-note {{caller function declared here}}
};
...
struct RefCountedSubChild : public RefCountedChild {
  ...
  method_test3() override; // expected-error {{functions marked as
                           // MOZ_CAN_RUN_SCRIPT cannot override functions that
                           // are not marked MOZ_CAN_RUN_SCRIPT}}
};

void RefCountedSubChild::method_test3() { // expected-error {{functions marked
                                          // as MOZ_CAN_RUN_SCRIPT cannot
                                          // override functions that are not
                                          // marked MOZ_CAN_RUN_SCRIPT}}
  test();
}
...
MOZ_CAN_RUN_SCRIPT void test6() {
  ...
  test2(
      (RefCountedBase *)
          x); // expected-error {{arguments must all be strong refs or caller's
              // parameters when calling a function marked as MOZ_CAN_RUN_SCRIPT
              // (including the implicit object argument).  'x' is neither.}}
}
...
MOZ_CAN_RUN_SCRIPT void test_ref_1() {
  RefCountedBase *t = new RefCountedBase;
  test_ref(
      *t); // expected-error {{arguments must all be strong refs or caller's
           // parameters when calling a function marked as MOZ_CAN_RUN_SCRIPT
           // (including the implicit object argument).  '*t' is neither.}}
}
...
MOZ_CAN_RUN_SCRIPT void test_ref_3() {
  ...
  auto &ref = *t;
  test_ref(
      ref); // expected-error {{arguments must all be strong refs or caller's
            // parameters when calling a function marked as MOZ_CAN_RUN_SCRIPT
            // (including the implicit object argument).  'ref' is neither.}}
}

MOZ_CAN_RUN_SCRIPT void test_ref_4() {
  RefCountedBase *t = new RefCountedBase;
  auto &ref = *t;
  ref.method_test(); // expected-error {{arguments must all be strong refs or
                     // caller's parameters when calling a function marked as
                     // MOZ_CAN_RUN_SCRIPT (including the implicit object
                     // argument).  'ref' is neither.}}
}
...
MOZ_CAN_RUN_SCRIPT void test_ref_9() {
  ...
  test_ref(*(RefCountedBase *)
               x); // expected-error {{arguments must all be strong refs or
                   // caller's parameters when calling a function marked as
                   // MOZ_CAN_RUN_SCRIPT (including the implicit object
                   // argument).  '*(RefCountedBase*)x' is neither.}}
}
...
MOZ_CAN_RUN_SCRIPT void test_maybe_2() {
  ...
  (*safe)
      ->method_test(); // expected-error-re {{arguments must all be strong refs
                       // or caller's parameters when calling a function marked
                       // as MOZ_CAN_RUN_SCRIPT (including the implicit object
                       // argument).  '(*safe){{(->)?}}' is neither.}}
}
...
MOZ_CAN_RUN_SCRIPT void test_defaults_helper_2(
    RefCountedBase *arg =
        new RefCountedBase()) { // expected-error {{arguments must all be strong
                                // refs or caller's parameters when calling a
                                // function marked as MOZ_CAN_RUN_SCRIPT
                                // (including the implicit object argument).
                                // 'new RefCountedBase()' is neither.}}
}
...
struct DisallowMemberArgs {
  ...
  MOZ_CAN_RUN_SCRIPT void foo() {
    mRefCounted
        ->method_test(); // expected-error-re {{arguments must all be strong
                         // refs or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'mRefCounted{{(->)?}}' is
                         // neither.}}
    ...
    unsafeMemberRef
        ->method_test(); // expected-error-re {{arguments must all be strong
                         // refs or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'unsafeMemberRef{{(->)?}}' is
                         // neither.}}
    ...
    maybeUnsafeMemberRef1
        ->method_test(); // expected-error-re {{arguments must all be strong
                         // refs or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'maybeUnsafeMemberRef1{{(->)?}}'
                         // is neither.}}
    ...
    maybeUnsafeMemberRef2
        ->method_test(); // expected-error-re {{arguments must all be strong
                         // refs or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'maybeUnsafeMemberRef2{{(->)?}}'
                         // is neither.}}
  }
  MOZ_CAN_RUN_SCRIPT void bar() {
    test2(mRefCounted); // expected-error {{arguments must all be strong refs or
                        // caller's parameters when calling a function marked as
                        // MOZ_CAN_RUN_SCRIPT (including the implicit object
                        // argument).  'mRefCounted' is neither.}}

    RefPtr<RefCountedBase> &unsafeMemberRef = mRefCounted;
    test2(unsafeMemberRef); // expected-error {{arguments must all be strong
                            // refs or caller's parameters when calling a
                            // function marked as MOZ_CAN_RUN_SCRIPT (including
                            // the implicit object argument).  'unsafeMemberRef'
                            // is neither.}}
    ...
    RefPtr<RefCountedBase> &maybeUnsafeMemberRef1 =
        mRefCounted ? mRefCounted : safeRefCounted;
    test2(
        maybeUnsafeMemberRef1); // expected-error {{arguments must all be strong
                                // refs or caller's parameters when calling a
                                // function marked as MOZ_CAN_RUN_SCRIPT
                                // (including the implicit object argument).
                                // 'maybeUnsafeMemberRef1' is neither.}}
    RefPtr<RefCountedBase> &maybeUnsafeMemberRef2 =
        safeRefCounted ? safeRefCounted : mRefCounted;
    test2(
        maybeUnsafeMemberRef2); // expected-error {{arguments must all be strong
                                // refs or caller's parameters when calling a
                                // function marked as MOZ_CAN_RUN_SCRIPT
                                // (including the implicit object argument).
                                // 'maybeUnsafeMemberRef2' is neither.}}
  }
};
...
struct DisallowMemberArgsWithGet {
  ...
  MOZ_CAN_RUN_SCRIPT void foo() {
    mRefCounted.get()
        ->method_test(); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'mRefCounted.get()' is neither.}}
  }
  MOZ_CAN_RUN_SCRIPT void bar() {
    test2(mRefCounted
              .get()); // expected-error {{arguments must all be strong refs or
                       // caller's parameters when calling a function marked as
                       // MOZ_CAN_RUN_SCRIPT (including the implicit object
                       // argument).  'mRefCounted.get()' is neither.}}
  }
};
...
struct DisallowMemberCallsOnRandomKnownLive {
  ...
  MOZ_CAN_RUN_SCRIPT void test_refptr_method() {
    MOZ_KnownLive(mWeakPtrReturner1)
        ...
        ->method_test(); // expected-error {{arguments must all be strong refs
                         ...
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).
                         // 'MOZ_KnownLive(mWeakPtrReturner1)->getWeakPtr()' is
                         // neither.}}
  }
  ...
  MOZ_CAN_RUN_SCRIPT void test_refptr_function() {
    test2(
        MOZ_KnownLive(mWeakPtrReturner1)
            ->getWeakPtr()); // expected-error {{arguments must all be strong
                             // refs or caller's parameters when calling a
                             // function marked as MOZ_CAN_RUN_SCRIPT
                             // (including the implicit object argument).
                             // 'MOZ_KnownLive(mWeakPtrReturner1)->getWeakPtr()'
                             // is neither.}}
  }
  ...
  MOZ_CAN_RUN_SCRIPT void test_raw_method() {
    MOZ_KnownLive(mWeakPtrReturner2)
        ...
        ->method_test(); // expected-error {{arguments must all be strong refs
                         ...
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).
                         // 'MOZ_KnownLive(mWeakPtrReturner2)->getWeakPtr()' is
                         // neither.}}
  }
    ...
    test2(
        MOZ_KnownLive(mWeakPtrReturner2)
            ->getWeakPtr()); // expected-error {{arguments must all be strong
                             ...
                             // function marked as MOZ_CAN_RUN_SCRIPT
                             // (including the implicit object argument).
                             // 'MOZ_KnownLive(mWeakPtrReturner2)->getWeakPtr()'
                             // is neither.}}
  }
};
...
struct DisallowConstMemberArgsOfMembers {
  ...
  MOZ_CAN_RUN_SCRIPT void foo() {
    mMember->mRefCounted
        ->method_test(); // expected-error-re {{arguments must all be strong
                         // refs or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'mMember->mRefCounted{{(->)?}}'
                         // is neither.}}
  ...
  MOZ_CAN_RUN_SCRIPT void bar() {
    test2(
        mMember->mRefCounted); // expected-error {{arguments must all be strong
                               // refs or caller's parameters when calling a
                               // function marked as MOZ_CAN_RUN_SCRIPT
                               // (including the implicit object argument).
                               // 'mMember->mRefCounted' is neither.}}
  ...
  MOZ_CAN_RUN_SCRIPT void foo() {
    mRefCounted
        ->method_test(); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'mRefCounted' is neither.}}
    ...
    unsafeMemberRefCounted
        ->method_test(); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'unsafeMemberRefCounted' is
                         // neither.}}
  }
  MOZ_CAN_RUN_SCRIPT void bar() {
    test2(mRefCounted); // expected-error {{arguments must all be strong refs or
    ...
    RefCountedBase *const &unsafeMemberRefCounted = mRefCounted;
    test2(unsafeMemberRefCounted); // expected-error {{arguments must all be
                                   // strong refs or caller's parameters when
                                   // calling a function marked as
                                   // MOZ_CAN_RUN_SCRIPT (including the implicit
                                   // object argument). 'unsafeMemberRefCounted'
                                   // is neither.}}
  }
};
...
MOZ_CAN_RUN_SCRIPT void test_temporary_4() {
  test_ref(*WeakSmartPtr(
      new RefCountedBase())); // expected-error {{arguments must all be strong
                              ...
                              // function marked as MOZ_CAN_RUN_SCRIPT
                              // (including the implicit object argument).
                              // '*WeakSmartPtr(new RefCountedBase())' is
                              // neither.}}
}
  ...
  MOZ_CAN_RUN_SCRIPT void foo() {
    mArray[0]
        ->method_test(); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'mArray[0]' is neither.}}
  }
  MOZ_CAN_RUN_SCRIPT void bar() {
    test2(mArray[0]); // expected-error {{arguments must all be strong refs or
                      // caller's parameters when calling a function marked as
                      // MOZ_CAN_RUN_SCRIPT (including the implicit object
                      // argument).  'mArray[0]' is neither.}}
  }
};
...
struct DisallowRefPtrTArrayElement {
  ...
  MOZ_CAN_RUN_SCRIPT void foo() {
    mArray[0]->method_test(); // expected-error-re {{arguments must all be
                              // strong refs or caller's parameters when calling
                              // a function marked as MOZ_CAN_RUN_SCRIPT
                              // (including the implicit object argument).
                              // 'mArray[0]{{(->)?}}' is neither.}}
  }
  MOZ_CAN_RUN_SCRIPT void bar() {
  ...
  MOZ_CAN_RUN_SCRIPT void foo() {
    ...
    bogus
        ->method_test(); // expected-error-re {{arguments must all be strong
                         // refs or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'bogus{{(->)?}}' is neither.}}
  }
  MOZ_CAN_RUN_SCRIPT void bar() {
    RefPtr<RefCountedBase> &bogus = mRefCounted;
    test2(bogus); // expected-error {{arguments must all be strong refs or
                  // caller's parameters when calling a function marked as
                  // MOZ_CAN_RUN_SCRIPT (including the implicit object
                  // argument).  'bogus' is neither.}}
  }
};
...
struct DisallowMemberArgsViaReferenceAlias2 {
  ...
  MOZ_CAN_RUN_SCRIPT void foo1() {
    for (RefPtr<RefCountedBase> &item : mRefCountedArr) {
      item->method_test(); // expected-error-re {{arguments must all be strong
                           // refs or caller's parameters when calling a
                           // function marked as MOZ_CAN_RUN_SCRIPT (including
                           // the implicit object argument).  'item{{(->)?}}' is
                           // neither.}}
    }
  }
  ...
  MOZ_CAN_RUN_SCRIPT void bar1() {
    for (RefPtr<RefCountedBase> &item : mRefCountedArr) {
      test2(item); // expected-error {{arguments must all be strong refs or
                   // caller's parameters when calling a function marked as
                   // MOZ_CAN_RUN_SCRIPT (including the implicit object
                   // argument).  'item' is neither.}}
    }
  }
...
MOZ_CAN_RUN_SCRIPT void AllowMozKnownLiveMemberInAutoStorage() {
  ...
  inStack.fooPtr(
      inHeap
          ->mWhatever); // expected-error {{arguments must all be strong refs or
                        // caller's parameters when calling a function marked as
                        // MOZ_CAN_RUN_SCRIPT (including the implicit object
                        // argument).  'inHeap->mWhatever' is neither.}}
  inStack.fooRef(
      *inHeap->mWhatever); // expected-error {{arguments must all be strong refs
                           // or caller's parameters when calling a function
                           // marked as MOZ_CAN_RUN_SCRIPT (including the
                           // implicit object argument).  '*inHeap->mWhatever'
                           // is neither.}}
  inStack.fooPtr(
      inHeap
          ->mRefCountedWhatever); // expected-error {{arguments must all be
                                  // strong refs or caller's parameters when
                                  // calling a function marked as
                                  // MOZ_CAN_RUN_SCRIPT (including the implicit
                                  // object argument).
                                  // 'inHeap->mRefCountedWhatever' is neither.}}
  inStack.fooRef(
      *inHeap->mRefCountedWhatever); // expected-error {{arguments must all be
                                     // strong refs or caller's parameters when
                                     // calling a function marked as
                                     // MOZ_CAN_RUN_SCRIPT (including the
                                     // implicit object argument).
                                     // '*inHeap->mRefCountedWhatever' is
                                     // neither.}}
  RefCountedBase *&whateverRefInHeap = inHeap->mWhatever;
  inStack.fooPtr(
      whateverRefInHeap); // expected-error {{arguments must all be strong refs
                          // or caller's parameters when calling a function
                          // marked as MOZ_CAN_RUN_SCRIPT (including the
                          // implicit object argument).  'whateverRefInHeap' is
                          // neither.}}
  inStack.fooRef(
      *whateverRefInHeap); // expected-error {{arguments must all be strong refs
                           // or caller's parameters when calling a function
                           // marked as MOZ_CAN_RUN_SCRIPT (including the
                           // implicit object argument).  '*whateverRefInHeap'
                           // is neither.}}
  RefPtr<RefCountedBase> &refCountedWhateverRefInHeap =
      inHeap->mRefCountedWhatever;
  inStack.fooPtr(
      refCountedWhateverRefInHeap); // expected-error {{arguments must all be
                                    // strong refs or caller's parameters when
                                    // calling a function marked as
                                    // MOZ_CAN_RUN_SCRIPT (including the
                                    // implicit object argument).
                                    // 'refCountedWhateverRefInHeap' is
                                    // neither.}}
  inStack.fooRef(
      *refCountedWhateverRefInHeap); // expected-error {{arguments must all be
                                     // strong refs or caller's parameters when
...
struct DisallowMozKnownLiveMemberNotFromKnownLive {
  ...
  MOZ_CAN_RUN_SCRIPT void fooRef(RefCountedBase &aWhatever) {}
  MOZ_CAN_RUN_SCRIPT void bar() {
    fooPtr(mMember->mWhatever); // expected-error {{arguments must all be strong
                                // refs or caller's parameters when calling a
                                // function marked as MOZ_CAN_RUN_SCRIPT
                                // (including the implicit object argument).
                                // 'mMember->mWhatever' is neither.}}
    fooRef(
        *mMember->mWhatever); // expected-error {{arguments must all be strong
                              // refs or caller's parameters when calling a
                              // function marked as MOZ_CAN_RUN_SCRIPT
                              // (including the implicit object argument).
                              // '*mMember->mWhatever' is neither.}}
    fooPtr(
        mMember->mRefCountedWhatever); // expected-error {{arguments must all be
                                       // strong refs or caller's parameters
                                       // when calling a function marked as
                                       // MOZ_CAN_RUN_SCRIPT (including the
                                       // implicit object argument).
                                       // 'mMember->mRefCountedWhatever' is
                                       // neither.}}
    fooRef(
        *mMember->mRefCountedWhatever); // expected-error {{arguments must all
                                        // be strong refs or caller's parameters
                                        // when calling a function marked as
                                        // MOZ_CAN_RUN_SCRIPT (including the
                                        // implicit object argument).
                                        // '*mMember->mRefCountedWhatever' is
    ...
    fooPtr(whateverRef); // expected-error {{arguments must all be strong refs
                         // or caller's parameters when calling a function
                         // marked as MOZ_CAN_RUN_SCRIPT (including the implicit
                         // object argument).  'whateverRef' is neither.}}
    fooRef(
        *whateverRef); // expected-error {{arguments must all be strong refs or
                       // caller's parameters when calling a function marked as
                       // MOZ_CAN_RUN_SCRIPT (including the implicit object
                       // argument).  '*whateverRef' is neither.}}
    RefPtr<RefCountedBase> &refCountedWhateverRef =
        mMember->mRefCountedWhatever;
    fooPtr(
        refCountedWhateverRef); // expected-error {{arguments must all be strong
                                // refs or caller's parameters when calling a
                                // function marked as MOZ_CAN_RUN_SCRIPT
                                // (including the implicit object argument).
                                // 'refCountedWhateverRef' is neither.}}
    fooRef(*refCountedWhateverRef); // expected-error {{arguments must all be
                                    // strong refs or caller's parameters when
                                    // calling a function marked as
                                    // MOZ_CAN_RUN_SCRIPT (including the
                                    // implicit object argument).
...
void IncorrectlyUnmarkedEarlyDeclaration(); // expected-note {{The first
                                            // declaration exists here}}

MOZ_CAN_RUN_SCRIPT void IncorrectlyUnmarkedEarlyDeclaration() {
}; // expected-error {{MOZ_CAN_RUN_SCRIPT must be put in front of the
