...
Global g2; // expected-error {{Global variable has runtime initialisation, try
           // to remove it, make it constexpr or constinit if possible, or as a
           // last resort flag it as MOZ_RUNINIT.}}
...
MOZ_RUNINIT InvalidRuninitGlobal
    g5; // expected-error {{Global variable flagged as MOZ_RUNINIT but actually
        // has constinit initialisation. Consider flagging it as constexpr or
        // constinit instead.}}
...
MOZ_RUNINIT InvalidRuninitGlobal2
    g5b; // expected-error {{Global variable flagged as MOZ_RUNINIT but actually
         // has constant initialisation. Consider removing the annotation or (as
         // a last resort) flagging it as MOZ_GLOBINIT.}}
InvalidRuninitGlobal2 g5c;

// Static variable with extern constructor
Global g6; // expected-error {{Global variable has runtime initialisation, try
           // to remove it, make it constexpr or constinit if possible, or as a
           // last resort flag it as MOZ_RUNINIT.}}
...
Global
    foobar::g9; // expected-error {{Global variable has runtime initialisation,
                // try to remove it, make it constexpr or constinit if possible,
