...
int main() {
  int *test =
      TemporaryFunction()
          .get(); // expected-error {{calling `get` on a temporary, potentially
                  // allowing use after free of the raw pointer}}
  int *test2 =
  ...
  int *escape;
  EscapeFunction2(
      TemporaryFunction().get(),
      escape); // expected-error {{calling `get` on a temporary, potentially
               // allowing use after free of the raw pointer}}
