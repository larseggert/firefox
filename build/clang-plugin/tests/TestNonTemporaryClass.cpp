...
void misuseNonTemporaryClass(int len) {
  ...
  gobbleref(
      NonTemporary(10, 20)); // expected-error {{variable of type 'NonTemporary'
                             // is not valid in a temporary}} expected-note
                             // {{value incorrectly allocated in a temporary}}
  gobbleref(
      NonTemporary(10)); // expected-error {{variable of type 'NonTemporary' is
                         // not valid in a temporary}} expected-note {{value
...
struct RandomClass {
  NonTemporary
      nonstaticMember; // expected-note {{'RandomClass' is a non-temporary type
                       // because member 'nonstaticMember' is a non-temporary
                       // type 'NonTemporary'}}
...
}; // expected-note {{'BadInherit' is a non-temporary type because it inherits
   // from a non-temporary type 'NonTemporary'}}
...
void useStuffWrongly() {
  gobbleanyref(BadInherit()); // expected-error {{variable of type 'BadInherit'
                              ...
                              // {{value incorrectly allocated in a temporary}}
  gobbleanyref(
      RandomClass()); // expected-error {{variable of type 'RandomClass' is not
                      // valid in a temporary}} expected-note {{value
                      // incorrectly allocated in a temporary}}
}
