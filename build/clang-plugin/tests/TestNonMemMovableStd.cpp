...
template <class T> class MOZ_NEEDS_MEMMOVABLE_TYPE Mover {
  T mForceInst;
}; // expected-error-re 10 {{Cannot instantiate 'Mover<{{.*}}>' with
...
template <class C>
class basic_string { // expected-note 2 {{'std::basic_string<char>' is a
                     // non-memmove()able type because it is an stl-provided
                     // type not guaranteed to be memmove-able}} expected-note
                     // {{'std::string' (aka 'basic_string<char>') is a
                     // non-memmove()able type because it is an stl-provided
                     // type not guaranteed to be memmove-able}}
public:
  basic_string();
...
template <class T, class U> class pair {
  T mT;
  U mU;
}; // expected-note-re 4 {{'std::pair<bool, {{.*}}>' is a non-memmove()able type
   // because it has a template argument non-memmove()able type '{{.*}}'}}

struct has_nontrivial_dtor { // expected-note 2 {{'std::has_nontrivial_dtor' is
                             // a non-memmove()able type because it is an
                             // stl-provided type not guaranteed to be
                             // memmove-able}}
  has_nontrivial_dtor() = default;
  ~has_nontrivial_dtor();
};
struct has_nontrivial_copy { // expected-note 2 {{'std::has_nontrivial_copy' is
                             // a non-memmove()able type because it is an
                             // stl-provided type not guaranteed to be
                             // memmove-able}}
...
struct has_nontrivial_move { // expected-note 2 {{'std::has_nontrivial_move' is
                             // a non-memmove()able type because it is an
                             // stl-provided type not guaranteed to be
                             // memmove-able}}
...
struct unique_ptr { // expected-note {{std::unique_ptr<std::basic_string<char>,
                    // custom_deleter<std::basic_string<char>>>' is a
                    // non-memmove()able type because it is an stl-provided type
                    // not guaranteed to be memmove-able}}
  T *ptr;
  D del;
  unique_ptr() = default;
  unique_ptr(T *p);
...
MOZ_RUNINIT static Mover<std::string>
    bad; // expected-note-re {{instantiation of 'Mover<std::basic_string<char>{{
         // ?}}>' requested here}}
MOZ_RUNINIT static Mover<HasString>
    bad_mem; // expected-note {{instantiation of 'Mover<HasString>' requested
             // here}}
static Mover<std::pair<bool, int>> good;
MOZ_RUNINIT static Mover<std::pair<bool, std::string>>
    not_good; // expected-note-re {{instantiation of 'Mover<std::pair<bool,
              // std::basic_string<char>{{ ?}}>{{ ?}}>' requested here}}

static Mover<std::has_nontrivial_dtor>
    nontrivial_dtor; // expected-note {{instantiation of
                     // 'Mover<std::has_nontrivial_dtor>' requested here}}
static Mover<std::has_nontrivial_copy>
    nontrivial_copy; // expected-note {{instantiation of
                     // 'Mover<std::has_nontrivial_copy>' requested here}}
static Mover<std::has_nontrivial_move>
    nontrivial_move; // expected-note {{instantiation of
                     // 'Mover<std::has_nontrivial_move>' requested here}}
...
static Mover<std::pair<bool, std::has_nontrivial_dtor>>
    pair_nontrivial_dtor; // expected-note {{instantiation of
                          // 'Mover<std::pair<bool, std::has_nontrivial_dtor>>'
                          // requested here}}
static Mover<std::pair<bool, std::has_nontrivial_copy>>
    pair_nontrivial_copy; // expected-note {{instantiation of
                          // 'Mover<std::pair<bool, std::has_nontrivial_copy>>'
                          // requested here}}
static Mover<std::pair<bool, std::has_nontrivial_move>>
    pair_nontrivial_move; // expected-note {{instantiation of
                          // 'Mover<std::pair<bool, std::has_nontrivial_move>>'
                          // requested here}}
...
static Mover<std::unique_ptr<std::string, custom_deleter<std::string>>>
    bad_unique_ptr; // expected-note {{instantiation of
                    // 'Mover<std::unique_ptr<std::basic_string<char>,
                    // custom_deleter<std::basic_string<char>>>>' requested
                    // here}}
